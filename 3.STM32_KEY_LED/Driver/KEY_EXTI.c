...
void EXTI4_IRQHandler(void)
{
    EXTI->PR |= EXTI_PR_PR4;
    delay_ms(20);
    if ((GPIOE->IDR & GPIO_IDR_IDR_4) == 0)
    {
        while((GPIOE->IDR & GPIO_IDR_IDR_4) == 0);
...
void EXTI3_IRQHandler(void)
{
    EXTI->PR |= EXTI_PR_PR3;
    delay_ms(20);
    if ((GPIOE->IDR & GPIO_IDR_IDR_3) == 0)
    {
        while((GPIOE->IDR & GPIO_IDR_IDR_3) == 0);
        LED_Toggle(LED_RED);
    }
}
...
void EXTI2_IRQHandler(void)
{
    EXTI->PR |= EXTI_PR_PR2;
    delay_ms(20);
    if ((GPIOE->IDR & GPIO_IDR_IDR_2) == 0)
    {
        while((GPIOE->IDR & GPIO_IDR_IDR_2) == 0);
        LED_OFF_ALL(LEDs, 2);
    }
}
